...
# --- 👑 MAIN PORTAL SCREEN (AFTER LOGIN) ---
    ...
    if not os.path.exists(EXCEL_FILE):
        ...
        # --- DASHBOARD LOGIC (RE-ARRANGED ORDER) ---
        else:
            ...
            # Left side: Date-wise Item Summary Box Table
            with col_left:
            ...
            # Right side: Date-wise Round Graph Donut Chart
            with col_right:
                st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
                ...
                if chart_values:
                    ...
                    st.info(f"Is date ({formatted_filter_date}) me graph ke liye koi valid production data entry nahi mili.")
                st.markdown('</div>', unsafe_allow_html=True)

            st.markdown("---")

            # --- 🎯 NO 2: TARGET STATUS TABLE (LATEST ENTRIES COUNTED FOR TOTAL LESS) ---
            ...
            for u_id, u_info in st.session_state["users"].items():
                if u_info["role"] == "supervisor":
                    ...
                    # Calculate total pieces till now by this supervisor
                    if excel_loaded and not df.empty:
