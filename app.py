...
if not st.session_state["logged_in"]:
    st.markdown('<h1 style="text-align:center;">COLOUR TEXTILE LOGIN</h1>', unsafe_allow_html=True)
    ...
    # Sidebar Navigation
    nav_options = ["📊 Dashboard", "📝 Data Entry", "📋 Manage Records"]
    if user["role"] == "admin":
    ...
    if choice == "📊 Dashboard":
        ...
        # Filter Logic without 'Filter Range Selection' text
        filter_type = st.radio("Select View:", ["Day-Wise", "Month-Wise"], horizontal=True)
        if filter_type == "Day-Wise":
            sel_date = st.date_input("Choose Date", datetime.now())
            data = df[df['Date'].dt.date == sel_date]
        else:
            sel_month = st.selectbox("Choose Month", df['Month_Year'].dropna().unique() if not df.empty else [])
            data = df[df['Month_Year'] == sel_month] if not df.empty else pd.DataFrame()

        c1, c2 = st.columns(2)
        with c1:
            st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
            st.markdown("### 🏭 ITEM PIECES SUMMARY")
            st.write(data.groupby('Item Type')['Total Pcs'].sum() if not data.empty else "No Data")
            st.markdown('</div>', unsafe_allow_html=True)
            
            # Party Pieces Summary (Added)
            st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
            st.markdown("### 🏢 PARTY PIECES SUMMARY")
            st.write(data.groupby('Party Name')['Total Pcs'].sum() if not data.empty else "No Data")
            st.markdown('</div>', unsafe_allow_html=True)

    # PAGE 2: EDITABLE RECORDS
    elif choice == "📋 Manage Records":
        st.header("📋 Manage Production Records (Edit Mode)")
        st.info("Aap table mein click karke data edit karein aur neeche 'Save' button dabayein.")
    ...
    # PAGE 3: FACTORY CONFIG (MOVED TO SIDEBAR)
    elif choice == "⚙️ Factory Config":
        st.header("⚙️ Factory Configurations Desk")
        t1, t2 = st.tabs(["Manage Parties", "Manage Items"])
        with t1:
            new_p = st.text_input("New Party Name")
            if st.button("Add Party"): st.session_state["party_options"].append(new_p); st.rerun()
        with t2:
            new_i = st.text_input("New Item Name")
            if st.button("Add Item"): st.session_state["item_options"].append(new_i.upper()); st.rerun()
import streamlit as st
import pandas as pd
import openpyxl
from datetime import datetime
